...
(function(){
  ...
  window.PavanDemoAuth = {
    login: async function(username,secret){
      ...
      let s=students().find(x=>String(x.username).toLowerCase()===u && x.passwordHash===hash); try{const cr=await fetch(SUPABASE_URL+'/rest/v1/student_credentials?select=username,password_hash,login_hash,active&username=eq.'+encodeURIComponent(username.trim())+'&limit=1',{headers:{apikey:SUPABASE_KEY,Authorization:'Bearer '+SUPABASE_KEY}});if(cr.ok){const rows=await cr.json();if(rows[0]&&rows[0].active===false)return null;if(rows[0]&&(rows[0].login_hash||rows[0].password_hash)===hash){const base=s||students().find(x=>String(x.username).toLowerCase()===u);if(base)s={...base,username:rows[0].username};else{try{const pr=await fetch(SUPABASE_URL+'/rest/v1/student_profiles?select=username,name,student_class,age,location,active&username=eq.'+encodeURIComponent(rows[0].username)+'&limit=1',{headers:{apikey:SUPABASE_KEY,Authorization:'Bearer '+SUPABASE_KEY}});const ps=pr.ok?await pr.json():[];if(ps[0]&&ps[0].active!==false)s={id:ps[0].id||Date.now(),name:ps[0].name,username:ps[0].username,studentClass:ps[0].student_class,age:ps[0].age,location:ps[0].location}}catch(e){}}}}}catch(e){console.warn('Credential service unavailable',e)}
      if(!s) return null;
